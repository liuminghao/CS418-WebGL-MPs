/**
 * @fileoverview Teapot - The Utah teapot 3D mesh for use with WebGL
 * @author Eric Shaffer
 * @author Minghao Liu
 */
    ...
    loadFromOBJ(fileText)
    {
      ...
      lines.forEach((line) => {
        if (line.startsWith('#')) {
          ...
          var literals = line.substr(2).split(" ");
          literals.forEach((literal) => {
            this.vBuffer.push(parseFloat(literal));
          });
          this.numVertices += 1;
        }
        else if (line.startsWith('f')) {
          var literals = line.substr(3).split(" ");
          literals.forEach((literal) => {
            this.fBuffer.push(parseInt(literal) - 1);
          });
          this.numFaces += 1;
        }
      });  
    ...
    */
    ...
    calculateXYZ() {
      ...
      for(var i=0;i<this.numVertices;i++)
          {
           this.vBuffer[i*3] > xmax ? xmax = this.vBuffer[i*3] : xmax = xmax;
           this.vBuffer[i*3] < xmin ? xmin = this.vBuffer[i*3] : xmax;
           this.vBuffer[i*3+1] > ymax ? ymax = this.vBuffer[i*3+1] : xmax;
           this.vBuffer[i*3+1] < ymin ? ymin = this.vBuffer[i*3+1] : xmax;
           this.vBuffer[i*3+2] > zmax ? zmax = this.vBuffer[i*3+2] : xmax;
           this.vBuffer[i*3+2] < zmin ? zmin = this.vBuffer[i*3+2] : xmax;
                       
          }
      
      this.xbar = (xmax + xmin) / 2;
      this.ybar = (ymax + ymin) / 2;
      this.zbar = (zmax + zmin) / 2;
      console.log(this.xbar + " " + this.ybar + " " + this.zbar);
    }
